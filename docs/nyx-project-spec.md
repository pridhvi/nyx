...
- **Benchmark validation** is opt-in with `NYX_RUN_BENCHMARKS=1`: run DVWA, OWASP Juice Shop, crAPI, OWASP Benchmark, DVGA, WebGoat, and NodeGoat against profile-owned setup data, route seeds, source extraction, and expected mappings. The summary gate enforces the accepted baseline by default for strict release gates: DVWA must cover at least 14 expected items, Juice Shop must cover at least 15 expected items, crAPI must cover 12 expected categories, OWASP Benchmark must cover 11 Java source-audit vulnerability classes from `expectedresults-1.2.csv`, DVGA must cover 24 upstream GraphQL solution categories, and those benchmark tool runs must have zero nonzero exits unless an explicit local override is set. OWASP Benchmark is currently a class-level Java source-audit gate and does not claim per-testcase precision across all 2,740 labeled cases. DVGA uses generic GraphQL review signals and does not execute destructive challenge payloads. WebGoat and NodeGoat start as baseline integrations with no minimum coverage gate until their accepted mappings are established.
