...
- **Benchmark validation** is opt-in with `NYX_RUN_BENCHMARKS=1`: run DVWA, OWASP Juice Shop, crAPI, OWASP Benchmark, DVGA, WebGoat, and NodeGoat against profile-owned setup data, route seeds, and expected mappings. The summary gate enforces the accepted Linux VM baseline by default for strict release gates: DVWA must cover at least 14 expected items, Juice Shop must cover at least 15 expected items, crAPI must cover 12 expected categories, and those benchmark tool runs must have zero nonzero exits unless an explicit local override is set. OWASP Benchmark, DVGA, WebGoat, and NodeGoat start as baseline integrations with no minimum coverage gate until their accepted mappings are established.
