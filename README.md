...
Benchmark artifacts are written under `artifacts/benchmarks/<timestamp>/`. DVWA, Juice Shop, crAPI, OWASP Benchmark, DVGA, and WebGoat are strict release gates: DVWA must cover at least 14 expected items, Juice Shop must cover at least 15 expected items, crAPI must cover 12 expected categories, OWASP Benchmark must cover all 11 labeled vulnerability classes from `expectedresults-1.2.csv`, DVGA must cover 24 upstream GraphQL solution categories, and WebGoat must cover 14 Java lesson-oriented categories. Those benchmark tool runs must avoid nonzero exits unless a local override is set. OWASP Benchmark starts the target for readiness and is measured as a Java source audit; the gate is class-level coverage and does not claim per-testcase precision across all 2,740 labeled cases. DVGA uses safe GraphQL probes plus schema-shaped human-assist review signals; it does not execute destructive challenge payloads. WebGoat registers a deterministic local benchmark user, validates authenticated lesson-menu access, downloads pinned WebGoat `v2025.3` source, and scores category-level Java source/authenticated evidence without hardcoding lesson solutions. NodeGoat remains a baseline integration first: it starts a local target, seeds routes, collects reports, and intentionally has no minimum coverage gate until its first accepted baseline is mapped.
